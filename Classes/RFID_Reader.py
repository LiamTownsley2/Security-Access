    ...
    def _start_reader(self):
        try:
            while True:
                ...
                self.logger.debug(f"start_reader() | Card Read Event -> (Tag ID: {id}) {text}")
                user = db.get_user_by_card(str(id))
                is_valid = self.validate_key(user, text)
                self.logger.info(f"*{'VALID' if is_valid else 'INVALID'} TAG READ* | ID: {id} | Text: '{text}'")

                if is_valid:
                    bucket, file_object = self.camera.record_and_upload(5, user['UserID'])

                    db.register_entry(str(id), user['UserID'], file_object)
                    entries = db.get_entries_count(user['UserID'])
                    self.logger.info(f"This employee has entered this building {entries} time(s) before.")
